...
1. 先读取与任务相关的 `STATUS.md`、下方当前开发策略、产品说明、架构说明、开发规范和适用的子目录 `AGENTS.md`。面向目标的迭代还须按 [`STATUS.md`](STATUS.md#冻结迭代目标) 定位冻结提交，读取该版本的 [`迭代目标`](docs/迭代目标.md)，明确本次对应的目标与验收编号。开发档位只选择执行默认值，不改变产品范围和已有硬约束。
