...
[`Docker Images`](../.github/workflows/docker-images.yml) 在推送 main、`v*` 标签或手动运行时发布应用镜像与三个独立插件镜像，支持 AMD64、ARM64、版本标签和 `sha-<完整提交 SHA>`。PR 只构建。手动运行可选 `app` 或任一插件；`all` 构建全部四个制品。Actions 使用仓库 `GITHUB_TOKEN`，无需服务器 SSH 密钥。

日常应用更新可使用 `ghcr.io/coachpo/signaldeck:main`，或设置明确版本/digest。插件须独立固定到已发布的完整 SHA 标签或 digest；这样普通 `pull` 不会将历史 endpoint 原地换成新插件。插件发布版本号与 Git 镜像标签不是同一概念。
...
保持在同一检出目录、使用同一份 env 和项目名。应用跟随 main 时通常只需：
...
应用的三个角色会使用同一个新镜像；固定版本的插件和已有数据库保持原制品/数据。应用固定版本时，先修改 `SIGNALDECK_IMAGE`，再执行上述命令。升级前核对是否另含数据库或 Temporal schema 变化：Core 的 `create_all` 只创建缺失表，不升级已有表，也不保证旧程序可读取新数据。普通 `stop`/`down` 保留命名卷，**不要运行 `down -v` 更新实例**。
