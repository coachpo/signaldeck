...
本文件定义 SignalDeck 后续迭代的目标架构与可观察验收，是目标内容的唯一权威。原始依据是引用任务[设计 Agent 平台理想架构](thread://01a07d7d-98fe-7c30-878a-7c8bec093ce9?hostId=local) 的最终结论及其最后修正；执行引擎选型按用户于 2026-09-08 在[冻结 Agent 平台迭代目标](thread://01a07d8d-d33c-76a0-9cb4-42510bef8eb4?hostId=local) 中明确作出的“确定使用 Temporal”决定更新，见 T06.2。
...
目标编号 `T01`—`T08` 和验收编号 `A01`—`A18` 保持不变。原冻结提交仍作为基线，执行引擎采用上述用户补充决定；Temporal 选型已定案，尚未完成的实现与集成验证仍不得写为已实现或已通过。
...
| 持久执行引擎 | Temporal；统一 Workflow、Agent 调用、计时、重试、取消与恢复 |
| Agent Runtime | Pydantic AI + TemporalDurability；保持统一 Agent 契约及调用级持久恢复能力 |
...
| 部署 | Docker Compose 组织 API、Worker、Temporal、数据库和业务插件 |
...
### T06.2：已确定使用 Temporal

用户于 2026-09-08 明确确定 **Temporal 为唯一持久执行引擎**，结束 Temporal、Prefect、Hatchet 之间的待定选型。该决定补充原冻结基线，不改变其他目标与验收要求。选型调研与用户决定来源见本文件开头的任务引用；三候选比较材料与实际探针的覆盖范围作为 A16 的实施交付证据保留，Prefect、Hatchet 不再作为待实现的产品执行路径。

选择 Temporal 是为了用同一执行生命周期承接 DAG 控制流程和 Agent 内部调用恢复，并复用 Pydantic AI 的官方持久执行集成。接受的代价是确定性执行约束、独立引擎运维，以及旧核心 Worker 制品的保留和路由。不能将其他候选探针尚未覆盖的场景视为其能力缺陷，也不能将本次选型决定视为完整集成验收通过。

目标执行映射为：一个 SignalDeck Run 对应一个 Temporal Workflow，Agent 节点执行对应 Child Workflow，模型请求或工具调用对应 Activity。Pydantic AI 的 `TemporalDurability` 与构造时注册、具有稳定 ID 的动态 Toolset 接入同一执行引擎；具体工具从冻结的 Run 契约与绑定解析。不得叠加第二套图执行引擎或自建持久恢复队列。

定时执行使用 Temporal Schedules，显式配置时区、重叠策略、错过策略和启动来源；其跟踪的 Workflow 生命周期必须覆盖实际工作完成，不能在仅提交启动命令后提前结束跟踪。

后续实现与验收集中验证 Temporal 组合：动态 schema、并发 Run 隔离、Agent 内部调用级恢复、父 Workflow→Agent Child Workflow→活动调用的完整取消链、跨重试总 deadline、固定插件及核心制品后的恢复、真实 MCP Streamable HTTP、outbox 中断与重复投递、同 Run 恢复与跨 Run 缓存区分，以及 Schedules 的实际生命周期。保留三候选比较证据及其限制，不因引擎选型结束而跳过 A16 或其他必需验收。
...
| A16 | T06 | 保留 Temporal、Prefect、Hatchet 在相同场景下的适配与运行成本比较证据，明确不同探针的覆盖范围和限制；按用户已确定的 Temporal 组合验证动态 schema、并发隔离、取消传播、调用级恢复和固定制品恢复，不把选型决定或未测能力记为验收通过。 |
