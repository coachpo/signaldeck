...
| S1 `sd-ux-task-form` | [任务页面](../../frontend/src/pages/platform/tasks.tsx)、[业务输入](../../frontend/src/pages/platform/task-inputs.tsx)；四场景表单、schema 不匹配提示专家输入 | P02；UX01/07；C1 | 完成：四场景实际业务表单运行；schema/默认值/无损输入回归及模式往返通过。 |
