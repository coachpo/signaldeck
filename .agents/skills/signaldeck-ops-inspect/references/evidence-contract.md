...
`--check` exits 1 when the repository version surfaces disagree, the deployment snapshot is unavailable, `app`, `db` or `temporal` is not healthy, `dispatcher` or `worker` is not running, the application roles run different images, the env file pins a different application image than the one running, `/ready` fails, or any container is restarting or exited non-zero. The application roles are the running services whose rendered Compose image follows the stack's version variable (`SIGNALDECK_VERSION`); plugin services run the same image repository but are pinned by their own variables, so they are not application roles. Discovery errors (missing project, ambiguous required container or an `app` image that does not follow the version variable) surface as an unavailable snapshot.
