...
4. **Plugin comparison** (informational): for each current plugin in `plugin-defaults.json`, pull the new plugin image and compare the running release descriptor with the new image described under the running service's environment, without network. `changed: true` means the release carries plugin changes that this rollout will not deploy.
