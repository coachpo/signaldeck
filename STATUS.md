...
### 工作流解耦（2026-09-09，本地未提交）

工作流解耦的本地实现已完成：所有保存的 Workflow 自动进入任务目录；schema/2 默认值与静态提示、presentation/1 标题/结果选择、resultLink/1 插件链接均为闭合公开合同；可选工作流 YAML 在 Core 可执行 closure 外通过原子 missing-only 导入。原包解析、编译、Launch、Temporal DAG/Agent 与独立插件执行边界保持，没有新增执行根、场景注册表或逐工作流前端扩展。

本轮按基线 `a3f5158eba369acda563f6eb73b7975b9b2a6785` 在独立 worktree 实施，核验接收 8 个交接文档；未修改原 checkout 或现有实例数据，未提交、推送、发布或部署。D01–D06、受影响 A01–A18、四场景及实际双离线读取的证据和命令见[解耦验证记录](docs/工作流解耦方案.md#验证记录)。最终测试数由该记录维护，不复用旧迭代计数。

无展示声明的历史记录保留通用确认值、产物与证据，不再按业务字段推测专用正文、标题或报告深链接。大产物选择可延后到附件阅读，不能解释为业务缺失。旧 hash、运行快照和历史 Core/插件清单不改写；本任务没有实施实例转换或数据处置。
