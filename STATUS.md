...
2026-09-20 核验时，capy 实例（部署仓库 `coachpo/curse` 的 `signaldeck` Compose 项目，拓扑见 [capy 适配说明](.agents/skills/signaldeck-ops-inspect/references/capy.md)）运行应用镜像 `ghcr.io/coachpo/signaldeck:v0.3.0@sha256:e0b79875dbed2b126f9d8a8e7c22a20b953e7f5e71e6b528d42aa11d2db671a3`，由该仓库 `signaldeck/backend.env` 的 `SIGNALDECK_VERSION` 固定。单镜像合并已经部署：应用与插件共用同一个制品，插件角色由 `sha-6198bd5c785b62f0e8a2f0cf6ad42673ab9bd343` 标签提供，各自的 `SIGNALDECK_FINANCE_VERSION`、`SIGNALDECK_NOTES_VERSION` 和 `SIGNALDECK_ORACLE_VERSION` 仍独立固定在 `backend.env`，不跟随 `SIGNALDECK_VERSION`。插件当前发布为 Finance 1.5.1（服务 `finance-6198bd5c`）、Digital Oracle 1.0.1（`digital-oracle-6198bd5c`）和 Notes 1.3.1（`notes-6198bd5c`），三者的 endpoint 经一次 `SIGNALDECK_BOOTSTRAP_REFRESH=1` 的 bootstrap 重新登记。没有 Run 绑定被取代的插件发布，旧服务 `finance-bb73a798`、`digital-oracle-b10785f8` 和 `notes-84c3ec46` 已从 Compose 中移除并在主机上删除，`legacy-plugins` profile 不再存在；它们在目录中的历史发布记录保留，页面挂载不再可达。实例导入了示例包 `watchlist_price_events`，定时任务 `watchlist-price-events-weekday-1645` 在纽约时区工作日 16:45 扫描 `finance-market-data` 中的自选股。以上是核验时的事实，不代表持续健康。
