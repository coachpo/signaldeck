...
- `includeHolders` and `includeEstimates` are separate opt-ins, also off by default. They read Yahoo's current holder structure and its current analyst consensus, target price and ratings, and reach the company analyst alone, as snapshots outside the shared evidence. The provider keeps no earlier versions, so a snapshot may post-date `asOfDate`, carries no `evidenceId` and can only support qualitative statements labelled as current; it never becomes a number, threshold or claim in the report, and the deterministic verification is unaffected.
