...
async function api(path,method='GET',body){let response;try{response=await window.SignalDeckUI.pluginFetch('api/'+path,{method,headers:body instanceof FormData?{}:{'Content-Type':'application/json'},body:body===undefined?undefined:body instanceof FormData?body:JSON.stringify(body)})}catch{throw new FinanceUiError(financeError(0,'',method))}if(!response.ok){let code='';try{code=(await response.json()).code}catch{}throw new FinanceUiError(financeError(response.status,code,method));}return response.status===204?null:response.json()}
...
function mutation(fn){return run(async(...args)=>{if(writing)return;writing=true;syncState();$('workspace').disabled=true;$('workspace').setAttribute('aria-busy','true');notice('正在处理，请稍候…');try{await fn(...args)}finally{writing=false;syncState();$('workspace').disabled=false;$('workspace').removeAttribute('aria-busy');if($('message').textContent==='正在处理，请稍候…')notice('')}})}
function syncState(){shell.setState([...drafts.values()].some(d=>d.name||d.content)||[...unsavedInputs].some(key=>Object.values(values.get(key)||{}).some(Boolean)),writing)}
function identity(){return tab+':'+(selected?.id??'new')}
function remember(){if(!$('author').hidden){const draft={name:$('name').value,content:editorContent()};if(selected&&draft.name===selected.name&&draft.content===selected.content)drafts.delete(identity());else drafts.set(identity(),draft);}if(!$('using').hidden)values.set(identity(),{...(values.get(identity())||{}),...inputValues()});queueMicrotask(syncState)}
...
$('download').onclick=run(async()=>{if(!selected)return;const response=await window.SignalDeckUI.pluginFetch('api/reports/'+encodeURIComponent(selected.slug)+'/download');if(!response.ok)throw new FinanceUiError('下载暂时不可用，请稍后重试。');const url=URL.createObjectURL(await response.blob()),link=document.createElement('a');link.href=url;link.download=selected.slug+'.md';link.click();setTimeout(()=>URL.revokeObjectURL(url),0)});
...
const restoreLocation=run(async()=>{const revision=++navigationGeneration;remember();reportEditing=false;invalidate();const params=new URLSearchParams(location.search),slug=params.get('report'),reportId=params.get('reportId');tab=params.get('tab')==='templates'?'templates':'reports';if(slug||reportId){const result=await api(slug?'reports/'+encodeURIComponent(slug):'reports/by-id/'+encodeURIComponent(reportId));if(revision!==navigationGeneration)return;selected=result;show()}else {selected=null;show()}await load();await loadReferences();syncState()});
window.addEventListener('popstate',restoreLocation);
document.addEventListener('input',()=>queueMicrotask(syncState));
document.addEventListener('click',()=>queueMicrotask(syncState));
restoreLocation();
