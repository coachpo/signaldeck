...
function renderInputs(){const saved=values.get(identity())||{};$('inputs').replaceChildren();for(const field of fields(content())){const label=document.createElement('label'),input=document.createElement('textarea');input.id='input-'+field.name;input.dataset.input=field.name;input.required=field.required;input.value=saved[field.name]||'';input.rows=2;input.style.minHeight="72px";label.htmlFor=input.id;label.textContent=field.label+(field.required?'（必填）':'（可选）');input.oninput=()=>{unsavedInputs.add(identity());values.set(identity(),{...(values.get(identity())||{}),...inputValues()});invalidate()};$('inputs').append(label,input)}if(!$('inputs').children.length){const p=document.createElement('p');p.textContent='此格式无需填写业务信息。';$('inputs').append(p)}}
