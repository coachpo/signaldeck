...
async function render() {
  ...
  try {
    ...
    const request = new URLSearchParams({collection, query: params.get('query') || '', limit: '20'});
...
$('search').addEventListener('submit', event => { event.preventDefault(); navigate(new URLSearchParams({collection: $('collection').value, query: $('query').value})); });
