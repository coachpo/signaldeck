...
Finance reads Yahoo quotes, history and OHLCV through the pinned `yfinance` library and keeps Yahoo's own bar labels: daily bars at the regular session start, weekly and monthly bars at exchange-local midnight. An OHLCV read requests Yahoo data up to the present and trims the window itself, because yfinance serves windows that ended in the past from an in-process cache. Yahoo news keeps Finance's own search client for the same reason: yfinance's search answers repeated identical queries from that cache.
