...
- `src/lib/api-client.ts` owns API base URL normalization, safe error details and downloads. Platform, ordinary-task and result clients under `src/lib/api/` use `requestPlatform` for `/api` endpoints. `src/lib/user-facing-error.ts` maps errors to supported recovery guidance; never render API codes, raw details or exception text directly. Independent plugins own their business pages.
