...
const outputLimitGuidance = "模型服务返回的输出超过本次上限，已停止该处理步骤的后续调用。已发生的用量仍保留；请核对本次输出长度限制与服务支持情况后再运行。";
...
export function ExecutionDiagnostic({ code, category }: {
  ...
  return <div className="flex min-w-0 flex-col gap-2 text-sm">
    <p>{message ?? "执行失败，具体原因未知。请查看执行过程，核对任务配置与服务状态后再运行。"}</p>

  </div>;
}
const labels = {
  not_observed: "当前连接还没有使用记录",
  succeeded: "最近使用成功",
  failed: "最近使用失败",
  unknown: "最近使用未确认",
};
export function ModelObservationDetails({ observation }: { observation?: ModelObservation | null }) {
  return <div className="flex min-w-0 flex-col gap-2 text-sm">
    ...
    <p className="text-muted-foreground">仅反映当前连接设置下已记录的使用情况，不代表此刻一定可用。</p>
    {observation?.errorCode && <ExecutionDiagnostic code={observation.errorCode} category={observation.errorCategory} />}
    {observation?.status === "unknown" && <p>调用结果未确认，请先查看执行过程再决定是否重试。</p>}
